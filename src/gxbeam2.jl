...
Model which describes the behavior of an assembly of beam elements, as modeled by the `GXBeam` package.  State variables are as defined by GXBeam.

Inputs correspond to the  external forces ``F_{x,i}, F_{y,i}, F_{z,i}, M_{x,i}, M_{y,i}, M_{z,i}`` or displacements ``u_{x,i}, u_{y,i}, u_{z,i}, \\theta_{x,i}, \\theta_{y,i}, \\theta_{z,i}`` applied to each node, followed by the distributed loads ``f_{x,i}, f_{y,i}, f_{z,i}, m_{x,i}, m_{y,i}, m_{z,i}`` applied to each beam element, followed by the point mass properties ``m, p, I11, I22, I33, I12, I13, I23`` corresponding to each beam element, followed by the gravity vector, linear velocity, angular velocity, linear acceleration, and angular acceleration of the system. 

Parameters correspond to the location ``p_{x}, p_{y}, p_{z}`` of each node followed by each beam element's properties. Each beam element's properties are defined by a triad which defines the orientation of the beam element ``e_{1,x}, e_{1,y}, e_{1,z}, e_{2,x}, e_{2,y}, e_{2,z}, e_{3,x}, e_{3,y}, e_{3,z}``, followed by the 21 independent entries of the compliance matrix 
...
"""
gxbeam(start, stop, displacement; kwargs...)
...
"""
function gxbeam(start, stop, displacement; force_scaling=1.0)
...
return gxbeam(start, stop, displacement, force_scaling,
...
as specified in a GXBeam `assembly` object and displacement constraints as specified in a
`prescribed_conditions` dictionary. (I think it's a dictionary.)

# Keyword Arguments
- `force_scaling = 1.0`: Factor used to scale forces and moments internally.  If not specified, a suitable default will be chosen based on the entries of the compliance matrix.
"""
function gxbeam(assembly, prescribed_conditions;
...
#Todo: Create a function that'll create a GXBeamAssembly from a blade file. (It looks like we don't need the cross sectional properties for this part of the game.)
...
# linear/angular velocity and acceleration #Todo: Do these match the values in the coupling? (Rather, does the coupling match these values?)
x0 = @SVector zeros(3)
...
function create_gxbeamfun(gxmodel)
# unpack constants
start = gxmodel.start
stop = gxmodel.stop
displacement = gxmodel.displacement
force_scaling = gxmodel.force_scaling
irow_point = gxmodel.irow_point
irow_elem = gxmodel.irow_elem
irow_elem1 = gxmodel.irow_elem1
irow_elem2 = gxmodel.irow_elem2
icol_point = gxmodel.icol_point
icol_elem = gxmodel.icol_elem

function gxbeamfun(outs, dx, x, p, t)
    gxbeam_residual!(outs, dx, x, y, p, t; start, stop, displacement, force_scaling, #What should start, stop, etc be? 
    irow_point, irow_elem, irow_elem1, irow_elem2, icol_point, icol_elem)
end
...
function differentialvars(gxmodel, n) #TODO: I might not need the n 
differential_vars = fill(false, length(x0))
for icol in gxmodel.icol_elem
...
e1x, e1y, e1z, e2x, e2y, e2z, e3x, e3y, e3z, C11, C12, C13, C14, C15, C16,
    ...
    C66, μ, xm2, xm3, i22, i33, i23 = p #Todo. This is different from what the struct header says. -> This is the section after the point. 
...
prescribed = Dict(ip => gxbeam_prescribed_condition(view(y,
    6*(ip-1) + 1 : 6*(ip-1) + 6), view(d, ip, :)) for ip = 1:np) #Note: changed view(d, :, ip) to view(d, ip, :)
...
point_masses = Dict(ie => gxbeam_point_mass(view(y,
    6*np + 6*ne + 10*(ie-1) + 1 : 6*np + 6*ne + 10*(ie-1) + 10)) for ie = 1:ne)  # I wonder if I need to just go through and update this to the new GXBeam.
# point_masses = Dict()   
...
f1_follower = f2_follower = @SVector zeros(3) #Todo: This means we aren't applying follower loads... do we want follower loads? 
...
# point mass
return PointMass(m, p, I) #Todo: Needs newer version of GXBeam. 
end

# convenience function for defining this model's state vector
#This is basically copied and pasted from GXBeam, with some minor changes. 
...
# convenience function for defining this model's parameter vector #Todo: Is there a better way to name icol_point, like what are these actually? 
function gxbeam_set_parameters!(p, icol_point, icol_elem; assembly) #Todo: What is the type of assembly? -> I think it is the actual GXBeam assembly. 
...
function gxbeam_element_velocities(V, Ω) #Todo: What's the point of this function? 
vi = V
ωi = Ω
return vi, ωi
end
