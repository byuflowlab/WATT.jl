...
if readflag
    ...
    # data = readdlm("./sn5_ADdriver.1.out", skipstart=8)
    data = readdlm("./sn5_input.out", skipstart=8)
    ...
    bdread = readdlm("./sn5_bddriver.out", skipstart=6)
...
tipdx = bdouts["TipTDxr"]
tipdy = bdouts["TipTDyr"]
tipdz = bdouts["TipTDzr"]
...
# plot!(rvec, dxmat[1,:], label="X OF", linestyle=:dash)
# plot!(rvec, dymat[1,:], label="Y OF", linestyle=:dash)
# plot!(rvec, dzmat[1,:], label="Z OF", linestyle=:dash)
# display(steadyplt)
