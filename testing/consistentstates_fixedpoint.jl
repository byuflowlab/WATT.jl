...
outs, state, system, assembly, prescribed_conditions, converged, iters, resids = fixedpoint(bemmodel, gxmodel, env, blade, p; maxiterations = 100, verbose = true, tolerance= 1e-12)
...
### Create GXBeam state function
fun = create_gxbeamfun(gxmodel, env, dsl, g=0.0, b=0.0)


fakeouts = zero(xs_gxbeam)
dxs_gxbeam = zero(xs_gxbeam)


fun(fakeouts, dxs_gxbeam, xs_gxbeam, p_s, 0.0)
