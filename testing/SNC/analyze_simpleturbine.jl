...
if defflag
    println("initialize...")
    aerostates, gxstates, gxhistory, mesh = Rotors.initialize(blade, assembly, tvec_r; verbose=true)
    defflag = false
end
...
runflag = true
if runflag #Ran up to 801 #Running well. 
    aerostates, gxstates, gxhistory = Rotors.simulate!(rotor_r, env, tvec_r, aerostates, gxstates, gxhistory, blade, mesh; verbose=true)
...
plot!(tiploads, tvec, fxmat[:,end], lab=L"$F_x$ - OF", seriescolor=:blue)
plot!(tiploads, tvec, -fymat[:,end], lab=L"$F_y$ - OF", seriescolor=:red)
...
plot!(tvec_r, -tipdef_z, lab=L"\delta x")
plot!(tvec_r, tipdef_y, lab=L"\delta y")
plot!(tvec_r, tipdef_x, lab=L"\delta z")
display(tipdefs2) #Todo: Align the graph styles. Dotted lines should be second. Use good style. 
