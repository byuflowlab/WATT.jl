...
precone = addriver.precone #0.0 #2.5*pi/180 #TODO: !!!! I need to work in a way to include precone
...
N, T, cchistory, xds = simulate(rvec, chordvec, twistvec, blade, env_steady, tvec, rhub, rtip, B, precone; verbose=false) #1.234718 seconds (5.51 M allocations: 262.152 MiB, 4.71% gc time, 83.64% compilation time) It's the createrisoode function that's taking time I think. There is probably a better way to do that. 
